...
def version() -> None:
	print("version 20240728")


def caustic2midi(inputFileName: str,inputData) -> None:
	input_data = inputData # just an ailias
	in_file = inputFileName
	
	if(inputFileName == "koisignal-test"):
		print("this is test")
		return
	print('DawVert: Daw Conversion Tool Caustic to midi mode')
	#utc = ZoneInfo("UTC")
	#local = ZoneInfo()
	...
	"""
	out_file = "output-" + now.strftime('%Y-%m-%d-%H-%M-%S') + ".midi"
	in_format = "caustic"
	out_format = "midi"
	...
	dawvert_core.parse_input(input_data, extra_json)
