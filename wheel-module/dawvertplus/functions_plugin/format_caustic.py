...
def deconstruct_main(input_file, useBytesIo = 1):# if useBytesio!=0, input_file is filepath
    if useBytesIo == 1:
        ro_rack = data_bytes.riff_read(input_file, 0)# file, offset 0だと先頭から取得
    else:
        fileobject = open(input_file, 'rb')
        headername = fileobject.read(4)
        ro_rack = data_bytes.riff_read(fileobject, 0)# file, offset 0だと先頭から取得
