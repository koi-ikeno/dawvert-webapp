...
from dawvertplus.plugin_plugconv_extern import base
...
from dawvertplus.functions_plugdata import data_wave

from dawvertplus.functions_plugdata import plugin_dragonfly_reverb
from dawvertplus.functions_plugdata import plugin_kickmess
from dawvertplus.functions_plugdata import plugin_vital
from dawvertplus.functions_plugdata import plugin_wolfshaper
from dawvertplus.functions_plugdata import plugin_socalabs
    ...
    def convert(self, cvpj_l, pluginid, cvpj_plugindata, extra_json, extplugtype):
        ...
        if 'nonfree-plugins' in extra_json:
            if flpluginname == 'fruity blood overdrive' and extplugtype == 'vst2':
                ...
                cvpj_plugindata.param_add('ext_param_0', paramvals[0], 'float', " PreBand  ")
                cvpj_plugindata.param_add('ext_param_1', paramvals[1], 'float', "  Color   ")
                cvpj_plugindata.param_add('ext_param_2', paramvals[2], 'float', "  PreAmp  ")
                cvpj_plugindata.param_add('ext_param_3', paramvals[3], 'float', "  x 100   ")
                cvpj_plugindata.param_add('ext_param_4', paramvals[4], 'float', "PostFilter")
                cvpj_plugindata.param_add('ext_param_5', paramvals[5], 'float', " PostGain ")

                auto_data.to_ext_one(cvpj_l, pluginid, 'preband', 'ext_param_0', 0, 10000)
                auto_data.to_ext_one(cvpj_l, pluginid, 'color', 'ext_param_1', 0, 10000)
                auto_data.to_ext_one(cvpj_l, pluginid, 'preamp', 'ext_param_2', 0, 10000)
                auto_data.to_ext_one(cvpj_l, pluginid, 'x100', 'ext_param_3', 0, 1)
                auto_data.to_ext_one(cvpj_l, pluginid, 'postfilter', 'ext_param_4', 0, 10000)
                auto_data.to_ext_one(cvpj_l, pluginid, 'postgain', 'ext_param_5', 0, 10000)
                return True
        ...
        # ---------------------------------------- DX10 ----------------------------------------
            ...
            cvpj_plugindata.param_add('ext_param_0', param_amp_att, 'float', "Attack  ", )
            cvpj_plugindata.param_add('ext_param_1', param_amp_dec, 'float', "Decay   ", )
            cvpj_plugindata.param_add('ext_param_2', param_amp_rel, 'float', "Release ", )
            cvpj_plugindata.param_add('ext_param_3', param_mod_course, 'float', "Coarse  ", )
            cvpj_plugindata.param_add('ext_param_4', param_mod_fine, 'float', "Fine    ", )
            cvpj_plugindata.param_add('ext_param_5', param_mod_init, 'float', "Mod Init", )
            cvpj_plugindata.param_add('ext_param_6', param_mod_time, 'float', "Mod Dec ", )
            cvpj_plugindata.param_add('ext_param_7', param_mod_sus, 'float', "Mod Sus ", )
            cvpj_plugindata.param_add('ext_param_8', param_mod_rel, 'float', "Mod Rel ", )
            cvpj_plugindata.param_add('ext_param_9', param_velsen, 'float', "Mod Vel ", )
            cvpj_plugindata.param_add('ext_param_10', param_vibrato, 'float', "Vibrato ", )
            cvpj_plugindata.param_add('ext_param_11', param_octave, 'float', "Octave  ", )
            cvpj_plugindata.param_add('ext_param_12', 0.5, 'float', "FineTune", )
            cvpj_plugindata.param_add('ext_param_13', param_waveform, 'float', "Waveform", )
            cvpj_plugindata.param_add('ext_param_14', param_mod_thru, 'float', "Mod Thru", )
            cvpj_plugindata.param_add('ext_param_15', param_lforate, 'float', "LFO Rate", )

            auto_data.to_ext_one(cvpj_l, pluginid, 'amp_att', 'ext_param_0', 0, 65536)
            auto_data.to_ext_one(cvpj_l, pluginid, 'amp_dec', 'ext_param_1', 0, 65536)
            auto_data.to_ext_one(cvpj_l, pluginid, 'amp_rel', 'ext_param_2', 0, 65536)
            auto_data.to_ext_one(cvpj_l, pluginid, 'mod_course', 'ext_param_3', 0, 65536)
            auto_data.to_ext_one(cvpj_l, pluginid, 'mod_fine', 'ext_param_4', 0, 65536)
            auto_data.to_ext_one(cvpj_l, pluginid, 'mod_init', 'ext_param_5', 0, 65536)
            auto_data.to_ext_one(cvpj_l, pluginid, 'mod_time', 'ext_param_6', 0, 65536)
            auto_data.to_ext_one(cvpj_l, pluginid, 'mod_sus', 'ext_param_7', 0, 65536)
            auto_data.to_ext_one(cvpj_l, pluginid, 'mod_rel', 'ext_param_8', 0, 65536)
            auto_data.to_ext_one(cvpj_l, pluginid, 'velsen', 'ext_param_9', 0, 65536)
            auto_data.to_ext_one(cvpj_l, pluginid, 'vibrato', 'ext_param_10', 0, 65536)
            auto_data.to_ext_one(cvpj_l, pluginid, 'octave', 'ext_param_11', -3, 3)

            auto_data.to_ext_one(cvpj_l, pluginid, 'waveform', 'ext_param_13', 0, 65536)
            auto_data.to_ext_one(cvpj_l, pluginid, 'mod_thru', 'ext_param_14', 0, 65536)
            auto_data.to_ext_one(cvpj_l, pluginid, 'lforate', 'ext_param_15', 0, 65536)
            return True
        ...
        # ---------------------------------------- SimSynth ----------------------------------------
            ...
            cvpj_plugindata.param_add('ext_param_0', param_freq, 'float', "Freq")
            cvpj_plugindata.param_add('ext_param_1', param_amount, 'float', "Weight")

            auto_data.to_ext_one(cvpj_l, pluginid, 'freq', 'ext_param_0', 0, 1280)
            auto_data.to_ext_one(cvpj_l, pluginid, 'amount', 'ext_param_1', 0, 1280)
            return True
            ...
            cvpj_plugindata.param_add('ext_param_0', 0, 'float', "attack")
            cvpj_plugindata.param_add('ext_param_1', 0, 'float', "release")
            cvpj_plugindata.param_add('ext_param_2', 0, 'float', "min env")
            cvpj_plugindata.param_add('ext_param_3', 0, 'float', "max env")
            cvpj_plugindata.param_add('ext_param_4', 0, 'float', "env-lfo mixture")
            cvpj_plugindata.param_add('ext_param_5', param_sweep_freq, 'float', "sweep freq.")
            cvpj_plugindata.param_add('ext_param_6', param_depth_min, 'float', "min. depth")
            cvpj_plugindata.param_add('ext_param_7', param_depth_max, 'float', "max. depth")
            cvpj_plugindata.param_add('ext_param_8', param_freq_range, 'float', "freq. range")
            cvpj_plugindata.param_add('ext_param_9', param_stereo, 'float', "stereo")
            cvpj_plugindata.param_add('ext_param_10', param_num_stages, 'float', "nr. stages")
            cvpj_plugindata.param_add('ext_param_11', 0, 'float', "distortion")
            cvpj_plugindata.param_add('ext_param_12', param_feedback, 'float', "feedback")
            cvpj_plugindata.param_add('ext_param_13', param_drywet, 'float', "dry-wet")
            cvpj_plugindata.param_add('ext_param_14', param_gain, 'float', "out gain")
            cvpj_plugindata.param_add('ext_param_15', 0, 'float', "invert")

            auto_data.to_ext_one(cvpj_l, pluginid, 'sweep_freq', 'ext_param_5', 0, 5000)
            auto_data.to_ext_one(cvpj_l, pluginid, 'depth_min', 'ext_param_6', 0, 1000)
            auto_data.to_ext_one(cvpj_l, pluginid, 'depth_max', 'ext_param_7', 0, 1000)
            auto_data.to_ext_one(cvpj_l, pluginid, 'freq_range', 'ext_param_8', 0, 1024)
            auto_data.to_ext_one(cvpj_l, pluginid, 'stereo', 'ext_param_9', 0, 1024)
            auto_data.to_ext_one(cvpj_l, pluginid, 'num_stages', 'ext_param_10', 0, 22)
            auto_data.to_ext_one(cvpj_l, pluginid, 'feedback', 'ext_param_12', 0, 1000)
            auto_data.to_ext_one(cvpj_l, pluginid, 'drywet', 'ext_param_13', 0, 1024)
            auto_data.to_ext_one(cvpj_l, pluginid, 'gain', 'ext_param_14', 0, 5000)
            return True
